...
const ReviewQuestionView = ({
  ...
  const getChoiceColor = (choice) => {
    ...
    const isCorrectChoice = correctAnswers.some(ca => ca.trim() === choiceTrimmed);
                ...
                {(!userAnswer || userAnswer.trim() === '') && 
                 ...
                 selfGradedScore === '0' && (
                  <div className="mb-3 p-2 bg-gray-50 border border-gray-200 rounded text-sm text-gray-700">
                    <span className="font-medium">Auto-scored:</span> This question was automatically scored as 0 (no answer or drawing provided).
                  </div>
                )}
                ...
                {(!userAnswer || userAnswer.trim() === '') && 
                 ...
                 (!selfGradedScore || selfGradedScore === '') && (
                  <div className="mb-3 p-2 bg-yellow-50 border border-yellow-200 rounded text-sm text-yellow-800">
                    <span className="font-medium">Note:</span> This question has no answer or drawing and will be auto-scored as 0 when you finish grading.
                  </div>
                )}
                ...
                <div className="space-y-2">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => onSelfGrade(question.id, 'score', e.target.value)}
                      placeholder={(!userAnswer || userAnswer.trim() === '') ? "0" : ""}
                    />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-blue-700 mb-1">
                    ...
                    <textarea
                      ...
                      onChange={(e) => onSelfGrade(question.id, 'comment', e.target.value)}
                    />
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gray-50 border border-gray-200 rounded p-3">
                ...
                <div className="space-y-1">
                  {selfGradedScore !== undefined && (
                    <p className="text-sm">
                      <span className="font-medium">Score:</span> {selfGradedScore}/{question.points} points
                    </p>
                  )}
                  {selfGradedComment && (
                    <p className="text-sm">
                      <span className="font-medium">Comments:</span> {selfGradedComment}
                    </p>
                  )}
                </div>
              </div>
...
const Review = () => {
          ...
          if (submissionData.test?.questions) {
            ...
            submissionData.test.questions.forEach(question => {
              if (question.type === 'saq' || question.type === 'leq') {
                const hasAnswer = submissionData.answer?.answers?.[question.id] && 
                                 submissionData.answer.answers[question.id].trim() !== '';
                const hasDrawing = submissionData.answer?.drawings?.[question.id] && 
                                  submissionData.answer.drawings[question.id].length > 0;
                const hasScore = autoScores[question.id] !== undefined && autoScores[question.id] !== '';
  ...
  const calculateMCQScore = () => {
    ...
    if (submission?.graded && submission?.totalScore !== undefined && submission?.maxScore !== undefined) {
    ...
    mcqQuestions.forEach(question => {
      ...
      if (isMultipleChoice) {
        // Multiple choice
        if (JSON.stringify(correctChoices.sort()) === JSON.stringify(userAnswers)) {
    ...
    return test.questions.filter(question => {
      ...
      if (filterType === 'unanswered') {
        if (question.type === 'mcq') {
          ...
          const hasDrawing = drawings[question.id] && drawings[question.id].length > 0;
  ...
  const checkMCQCorrect = (question) => {
    ...
    if (isMultipleChoice) {
      ...
      return JSON.stringify(correctChoices.sort()) === JSON.stringify(userAnswers);
    } else {
  ...
  const handleFinishGrading = async () => {
    ...
    // Auto-score unanswered questions with no drawings as 0
    const saqLeqQuestions = test.questions.filter(q => q.type === 'saq' || q.type === 'leq');
    const updatedScores = { ...selfGradedScores };
    
    saqLeqQuestions.forEach(question => {
      const hasAnswer = answers[question.id] && answers[question.id].trim() !== '';
      const hasDrawing = drawings[question.id] && drawings[question.id].length > 0;
      const hasScore = updatedScores[question.id] !== undefined && updatedScores[question.id] !== '';
    ...
    const missingScores = saqLeqQuestions.filter(q => {
      ...
      const hasScore = updatedScores[q.id] !== undefined && updatedScores[q.id] !== '';
    ...
    if (missingScores.length > 0) {
      setError(`Please provide scores for all answered SAQ/LEQ questions. Missing scores for ${missingScores.length} question(s).`);
      return;
    }
      ...
      <div className="max-w-6xl mx-auto bg-white rounded-lg shadow-md py-4 px-6 space-y-4">
        ...
        <div className="flex justify-between items-center">
          <h2 className="text-2xl font-bold text-orange-800">
            {test.random ? 'Random Test' : `${test.school} ${test.year}`} — {test.event} Review
          </h2>
          <div className="text-right">
            {submission.graded ? (
              ...
              <div className="text-orange-600 font-semibold">
                <div>MCQ Score: {mcqScores.score}/{mcqScores.total}</div>
                <div className="text-sm text-orange-500">Complete self-grading to see total score</div>
              </div>
            )}
          </div>
        </div>
        ...
        <div className={`flex space-x-4 ${submission.graded ? 'h-[500px]' : 'h-[400px]'}`}>
          ...
          >
            <ReviewQuestionView
              ...
              isCorrect={currentQuestion.type === 'mcq' ? checkMCQCorrect(currentQuestion) : null}
        ...
        <div className="flex justify-between items-center">
          <div className="flex gap-2 items-center min-w-[120px]">
            ...
            {filteredQuestions[currentIdx] && drawings[filteredQuestions[currentIdx].id] && drawings[filteredQuestions[currentIdx].id].length > 0 && (
              <button
