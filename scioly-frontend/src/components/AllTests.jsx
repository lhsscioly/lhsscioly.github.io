  ...
  useEffect(() => {
    const fetchAllData = async () => {
      ...
      try {
        ...
        const years = [...new Set(allSubmissions.map(sub => sub.schoolYear).filter(Boolean))]
          .sort((a, b) => {
  ...
  const getFilteredSubmissions = () => {
    return submissions.filter(submission => {
      ...
      if (searchTerm.trim()) {
        ...
        const testName = `${submission.test?.school || ''} ${submission.test?.year || ''} ${submission.test?.event || ''}`.toLowerCase();
        ...
        if (students && Array.isArray(students)) {
          studentNames = students
            .filter(student => student && typeof student === 'object' && student.firstName && student.lastName)
            .map(student => `${student.firstName} ${student.lastName}`.trim())
            .join(' ').toLowerCase();
        }
        
        if (!testName.includes(search) && !teamName.includes(search) && !studentNames.includes(search)) {
          return false;
        }
      }
    ...
    <div className="max-w-4xl mx-auto mt-8 p-6 bg-white rounded-lg shadow-md space-y-8">
      ...
      <div className="bg-orange-50 p-4 rounded-lg space-y-4">
        <div className="flex flex-wrap gap-4 items-center">
          ...
          {/* School Year Filter */}
          <div className="flex items-center gap-2">
            <label className="text-sm font-medium text-orange-700">School Year:</label>
            <select
          ...
          <div className="flex items-center gap-2 flex-1 min-w-[200px]">
            <label className="text-sm font-medium text-orange-700">Search:</label>
            <input
              type="text"
              value={searchTerm}
        ...
        <>
          ...
          <div>
            ...
            {filteredSubmissions.filter(s => s.graded).length === 0 ? (
              <p className="text-gray-500 italic pl-4">No graded tests match your filters.</p>
            ) : (
              <ul className="space-y-3">
                {filteredSubmissions.filter(s => s.graded).map((submission) => (
                  <li
                    key={submission.id}
                    ...
                    <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-2">
                      <div>
                        ...
                        <p className="text-xs text-green-600">
                          Team: {submission.team?.name} • Score: {submission.totalScore}/{submission.maxScore} ({((submission.totalScore/submission.maxScore)*100).toFixed(1)}%)
                        </p>
                        <p className="text-xs text-gray-500">
                          Students: {(() => {
                            ...
                            return names.length > 0 ? names.join(', ') : 'No students with names found';
                          })()}
                        </p>
                        <p className="text-xs text-gray-500">
          ...
          <div>
            <h3 className="text-xl font-medium text-orange-700 mb-4 text-left border-b border-orange-200 pb-2">
              Ungraded Tests ({filteredSubmissions.filter(s => !s.graded).length})
            </h3>
            {filteredSubmissions.filter(s => !s.graded).length === 0 ? (
              <p className="text-gray-500 italic pl-4">No ungraded tests match your filters.</p>
            ) : (
              <ul className="space-y-3">
                {filteredSubmissions.filter(s => !s.graded).map((submission) => (
                  <li
          ...
          <div>
            ...
            {getAssignedTests().length === 0 ? (
              <p className="text-gray-500 italic pl-4">No tests have been assigned to teams.</p>
            ) : (
              <ul className="space-y-3">
                  ...
                  >
                    <div>
                      ...
                      <p className="text-xs text-red-600">
                        Assigned to {test.assignees.length} team(s): {test.assignees && test.assignees.length > 0 
                          ? test.assignees.map(team => team.name || team).join(', ')
                          : 'No teams'
                        }
                      </p>
                    </div>
