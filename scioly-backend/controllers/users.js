...
usersRouter.post('/', async (request, response) => {
    ...
    if (!password) {
        ...
        return response.status(400).json({ error: 'Path `password` is shorter than the minimum allowed length (8)' })
    }
        ...
        try {
            await transporter.sendMail({
                ...
                html: `
                    ...
                    <a href="http://localhost:${config.PORT}/api/users/verify?token=${verificationToken}">Verify Email</a>
                `
            })
        } catch (error) {
...
usersRouter.put('/reset/:id', userExtractor, async (request, response) => {
    ...
    if (request.user.id.toString() !== id.toString()) {
      return response.status(403).json({ error: 'unauthorized to change this user' })
    }
